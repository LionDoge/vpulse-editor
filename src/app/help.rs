...
pub fn help_hover_text<'a>(template: PulseNodeTemplate, _user_state: &PulseGraphState) -> Cow<'a, str> {
    match template {
        ...
        PulseNodeTemplate::DebugWorldText => "Displays a debug text on a given entity. Works only in development environment.",
        ...
        PulseNodeTemplate::ForLoop => "A ranged loop with an index, the range is inclusive of the 'to' value.",
        ...
        PulseNodeTemplate::CompareOutput => "Compares two values, returns a condition, which can be used for nodes that require conditions i.e 'If' \
            Supported operations: ==, !=, <, >, <=, >=. Note that string comparison here is case-insensitive. There exists a separate node for case-sensitive comparison.",
        ...
        PulseNodeTemplate::NewArray => "Creates a new array of the provided type. You can also add initial values if applicable to the type, otherwise they may be added later at runtime.",
